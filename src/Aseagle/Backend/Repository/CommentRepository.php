...
class CommentRepository extends EntityRepository {
    ...
    public function getCommentsByPostId($cid, $order, $limit, $offset, $count=false) 
    {
        $query = $this->createQueryBuilder('o');
        if ($count) {
            $query->select('COUNT(o.id)');
        } else {
            $query->select('o.id, o.fullname, o.email, o.comment, p.id as parent_id, c.id as cid');
        }
        
        $query->leftJoin('o.content', 'c')
              ->leftJoin('o.parent', 'p')
              ->where('c.id = :cid')->setParameter(':cid', $cid);
